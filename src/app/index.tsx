...
export default function Index() {
  ...
  function handleNextStepForm() {
    if (destination.length === 0 || !selectedDates.startsAt || !selectedDates.endsAt) {
    ...
    <View className="flex-1 items-center justify-center px-5">
      ...
      <View className="w-full bg-zinc-900 p-4 rounded-lg my-8 border border-zinc-800">
        <Input>
          ...
          <Input.Field placeholder="Para onde?" editable={stepForm === StepForm.TRIP_DETAILS} value={destination}
            onChangeText={setDestination} style={{ color: colors.zinc[300] }}
            autoCorrect={false}
          />
        </Input>
        ...
        <Input>
          ...
          <Input.Field placeholder="Quando?" editable={stepForm === StepForm.TRIP_DETAILS} value={selectedDates.formatDatesInText}
            style={{ color: colors.zinc[300] }}
            onFocus={() => Keyboard.dismiss()}
            showSoftInputOnFocus={false}
            onPressIn={() => stepForm === StepForm.TRIP_DETAILS && setShowModal(ModalContent.CALENDAR)}
